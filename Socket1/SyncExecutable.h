...
public:
	...
	R DoSync(R(T::*memfunc)(Args...), Args... args)
	{
		...
		// TODO : mLock으로 보호한 상태에서 memfunc 실행하고 결과값 R을 리턴
	}
	...
	{
		static_assert(true == std::is_convertible<T, SyncExecutable>::value, "T should be derived from SyncExecutable");
		// TODO : this 포인터를 std::shared_ptr<T>형태로 변환
		// 이 클래스는 std::enable_shared_from_this에서 상속 받았다. 그리ㅗㄱ static_pointer_cast 사용

		return std::shared_ptr<T>((Player*)this); // 작동안함
	}
